...
// sessions
app.use(session({
    secret: "X^%9c2E!g*#45aQ1", cookie: {maxAge: 10800000}  // chave secreta para gerar a sessão
}))
...
app.get("/category/:slug", (req, res) => {
    ...
    Category.findOne({
        ...
        if(category != undefined){
            Category.findAll().then(categories => {
                res.render("index", {articles: category.articles,categories: categories});  //Só é possível acessar o articles porque foi feito o relacionamento entre as tabelas
            });
        }else{
            res.redirect("/");
        }
